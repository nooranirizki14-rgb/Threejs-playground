...
// GPU rain: all motion computed in the vertex shader, zero per-frame CPU cost.
// A box of streaks + splash rings that follows the camera.
...
export class RainStreaks {
  constructor(scene, { count = 2600 } = {}) {
    const geo = instancedQuads(count, 4, (d, n, s) => {
      for (let i = 0; i < n; i++) {
        d[i * s] = Math.random();          // x 0..1 in box
        d[i * s + 1] = Math.random();      // z 0..1 in box
        d[i * s + 2] = Math.random();      // y 0..1
        d[i * s + 3] = 17 + Math.random() * 9; // fall speed
      }
    });
    this.uniforms = {
      uTime: { value: 0 },
      uCamPos: { value: new THREE.Vector3() },
      uBox: { value: new THREE.Vector2(60, 60) },
      uHeight: { value: 30 },
      uWidth: { value: 0.035 },
      uLen: { value: 1.0 },
      uSlant: { value: 0.16 },
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: /* glsl */`
        ...
        void main() {
          ...
          // screen-aligned streak with wind shear
          mv.xy += vec2(position.x * uWidth + position.y * uSlant * uLen,
                         position.y * uLen);
          gl_Position = projectionMatrix * mv;
          vAlpha = (1.0 - smoothstep(25.0, 58.0, dist)) * smoothstep(0.4, 3.0, dist);
        }
      `,
      fragmentShader: /* glsl */`
        uniform vec3 uColor;
  ...
  constructor(scene, { count = 350 } = {}) {
    const geo = instancedQuads(count, 4, (d, n, s) => {
      for (let i = 0; i < n; i++) {
        d[i * s] = Math.random();
        d[i * s + 1] = Math.random();
        d[i * s + 2] = Math.random();          // phase
        d[i * s + 3] = 1.2 + Math.random() * 1.6; // cycle speed
      }
    });
    this.uniforms = {
      uTime: { value: 0 },
