...
const GradeShader = {
  ...
  fragmentShader: /* glsl */`
    ...
    void main() {
      ...
      // gentle filmic S-curve + lifted blacks
      c.rgb = c.rgb * (0.92 + 0.16 * c.rgb);
      c.rgb += vec3(0.012, 0.014, 0.02);
      // split tone: cool shadows, warm highlights
      float lum = dot(c.rgb, vec3(0.299, 0.587, 0.114));
      c.rgb += (vec3(0.02, 0.03, 0.055) * (1.0 - lum) + vec3(0.045, 0.028, 0.008) * lum) * (1.0 - lum * 0.4);
      // rich mids
      c.rgb = mix(vec3(lum), c.rgb, 1.06);
