...
export class Game {
  constructor() {
    ...
    this.overlay = null;  // shop | rest | help | null
    this.cash = 500000;
    this.fuel = 12;
    this.odoM = 0;
    this.clockMin = 0;
  ...
  updateDrive(dt, t) {
    ...
    if (this.engineRunning()) {
      ...
      const rate = 8 + throttle * 14 + (kmh > 100 ? 6 : 0);
    ...
    if (this.fuel < TANK * 0.15 && !this.flagFuel && this.fuel > 0) {
      this.flagFuel = true;
      this.hud.toast('⛽ Low fuel — find an SPBU soon!', 3200);
    }
    if (this.fuel >= TANK * 0.2) this.flagFuel = false;
  ...
  updateInteract(dt) {
    ...
    if (cur.id.startsWith('zone:')) {
      ...
      if (zn.type === 'shop') {
        ...
        this.garageHold(dt, eHeld, jp, zn);
  ...
  flapHold(dt, eHeld) {
    ...
    const rate = 1.5; // L/s
    const add = Math.min(rate * dt, TANK - this.fuel);
    ...
    this.hud.progress(this.fuel / TANK,
  ...
  garageHold(dt, eHeld, jp, zn) {
    const carD = Math.hypot(this.phy.x - zn.x, this.phy.z - zn.z);
    if (carD > 16) {
      this.setPrompt('Bring the car closer to the garage 🚗');
      return;
    }
    if (this.engine.health >= 99.5) {
      this.setPrompt('Engine is healthy ✨');
    ...
    const cost = Math.max(25000, Math.round((100 - this.engine.health) * 2500));
    this.setPrompt(`<b>HOLD E</b> — Full service · ${fmtRp(cost)}`);
    if (!eHeld) {
      this.hud.progress(null);
      return;
    }
    if (this.cash < cost) {
      this.setPrompt(`Need ${fmtRp(cost)} — push on, champ 💸`);
      if (jp.has('KeyE')) this.audio.deny();
      this.hud.progress(null);
      return;
    }
    this.holdT += dt;
    if (Math.random() < dt * 2) this.audio.clank();
    this.hud.progress(this.holdT / 3.5, '🔧 Full service…');
    if (this.holdT >= 3.5) {
      this.cash -= cost;
      this.engine.repairGarage();
      this.audio.repairDone();
      this.hud.toast('Full service done! Engine 100% ✨ ' + fmtRp(cost), 3000);
      this.holdId = '@@done@@';
      this.hud.progress(null);
    }
  }
  ...
  useItem(n) {
    ...
    if (n === 1) {
      ...
      this.inv.can -= 1;
      this.fuel = Math.min(TANK, this.fuel + 15);
  ...
  updateCamera(dt) {
    ...
    if (this.state === 'drive') {
      ...
      this.camera.rotation.set(pitch + sy * 0.1, yaw + sx * 0.1, 0);
    } else {
