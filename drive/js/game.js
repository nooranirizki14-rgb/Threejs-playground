...
export class Game {
  constructor() {
    ...
    this.pushing = false;
    this.lookBack = false;
    this.camMode = 0; // 0 driver · 1 hood · 2 chase
    this.shake = 0;
    this.crashCd = 0;
    this.scrapeT = 0;
    this.nodT = 20;
  ...
  update(dt) {
    ...
    this.world.hemi.intensity = 0.3 + flash * 2.6;
    this.world.moon.intensity = 0.22 + flash * 1.8;
    ...
    this.audio.updateBeds(kmh, this.weather.intensity, Math.abs(this.phy.slide) > 1.6,
      this.surfaceAt(this.phy.x, this.phy.z) === 'dirt' ? 1 : 0);
  ...
  surfaceAt(x, z) {
    if (Math.abs(x) < 8.6) return this.road.dirtAt(z) ? 'dirt' : 'road';
  ...
  cycleCam() {
    ...
    this.audio.click();
    this.hud.toast(['🚗 Driver cam', '🧢 Hood cam', '🎥 Chase cam'][this.camMode], 1500);
  }
    ...
    if (this.state === 'drive') {
      ...
      if (this.camMode === 1) {
        ...
        this.camera.position.set(
          this.phy.x + f.x * 2.1 + sx * 0.2, 1.08 + sy * 0.2, this.phy.z + f.z * 2.1);
        this.camera.rotation.set(clamp(this.controls.lookPitch, -0.4, 0.25),
          this.phy.heading + clamp(this.controls.lookYaw, -1.2, 1.2), 0);
      } else if (this.camMode === 2) {
        // chase cam
        this.camera.position.set(
          this.phy.x - f.x * 8 + sx * 0.3, 3.1 + sy * 0.3, this.phy.z - f.z * 8);
        this.camera.lookAt(this.phy.x + f.x * 6, 1, this.phy.z + f.z * 6);
      } else {
        this.camera.position.set(
