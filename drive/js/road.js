...
import { makeRoadBoard, makeGlowSprite, makeDetail } from './textures.js';
...
export class Road {
  ...
  layoutDashes(pz) {
    ...
    for (let i = 0; i < this.dashSpots.length; i++) {
      ...
      this.dashZ[i] = z;
      d.position.set(s.x, this.dirtAt(z) ? -5 : 0.015, z);
      d.scale.set(1, 1, 1);
  ...
  layoutLamps(pz) {
    ...
    for (let i = 0; i < this.lampZ.length; i++) {
      ...
      const dark = this.dirtAt(this.lampZ[i]);
      d.position.set(x, dark ? -5 : 4.5, this.lampZ[i]);
      d.scale.set(1, 1, 1);
      d.rotation.set(0, 0, 0);
      d.updateMatrix();
      this.poles.setMatrixAt(i, d.matrix);
      d.position.set(x - this.lampSide[i] * 1.2, dark ? -5 : 9, this.lampZ[i]);
      d.updateMatrix();
      this.lampHeads.setMatrixAt(i, d.matrix);
      d.position.set(x - this.lampSide[i] * 1.5, dark ? -5 : 0.03, this.lampZ[i]);
  ...
  updateSpots(pz) {
    ...
    for (let i = 0; i < this.lampZ.length; i++) {
      ...
      if (dz > 8 && dz < 200 && !this.dirtAt(this.lampZ[i])) ahead.push({ dz, i });
    }
  ...
  // ---- dirt stretches: unpaved country segments (no paint, no lamps) ----
  dirtAt(z) {
    if (z > -500) return false; // paved near the start
    const t = -z - 750;
    const k = Math.round(t / 1500);
    if (k % 4 !== 3) return false; // every 4th inter-station gap
    return Math.abs(t - k * 1500) < 500; // 1 km of dirt
  }

  buildDirt() {
    const geo = new THREE.PlaneGeometry(17.4, 1000);
    const tex = makeDetail({ base: '#221a10', spots: 2600 });
    tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
    tex.repeat.set(2, 120);
    const mat = new THREE.MeshStandardMaterial({ map: tex, color: 0xbfae90, roughness: 1 });
    this.dirtPatches = [];
    for (let i = 0; i < 3; i++) {
      ...
      m.visible = false;
      this.scene.add(m);
      this.dirtPatches.push(m);
    }
  }

  layoutDirt(pz) {
    const kc = Math.round((-pz - 750) / 1500);
    let pi = 0;
    for (let k = kc - 5; k <= kc + 5 && pi < 3; k++) {
      if (k < 3 || k % 4 !== 3) continue;
      const cz = -(k * 1500 + 750);
      if (cz - pz > 150 || cz - pz < -1350) continue;
      const m = this.dirtPatches[pi++];
      m.visible = true;
      m.position.z = cz;
    }
    for (; pi < 3; pi++) this.dirtPatches[pi].visible = false;
  }
}
