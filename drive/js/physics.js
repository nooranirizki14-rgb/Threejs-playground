...
export class CarPhysics {
  ...
  update(dt, input, env) {
    ...
    const surfMax = (env.surface === 'road' ? 56 : env.surface === 'lot' ? 22 : 11) + (env.topBoost || 0);
    const surfGrip = Math.min(1.25,
      (env.surface === 'road' ? 1 : env.surface === 'lot' ? 0.85 : 0.5) + (env.gripBoost || 0));
    ...
    if (env.engineOn) {
      if (input.throttle > 0 && !this.reversing) {
        ...
        const pull = 8.5 * power * (1 - Math.max(0, this.speed) / (top + 8));
